...
class FastGradientMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        return fgm(x, self.model(x), y=self.y, eps=self.eps, ord=self.ord,
                   clip_min=self.clip_min, clip_max=self.clip_max, defense_models=self.defense_models)
    ...
    def parse_params(self, eps=0.3, ord=np.inf, y=None, clip_min=None,
                     clip_max=None, defense_models=None, **kwargs):
        """
        Take in a dictionary of parameters and applies attack-specific checks
        before saving them as attributes.
        ...
        self.defense_models = defense_models
...
class BasicIterativeMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        for i in range(self.nb_iter):
            FGSM = FastGradientMethod(self.model, back=self.back, sess=self.sess)
    ...
    def parse_params(self, eps=0.3, eps_iter=0.05, nb_iter=10, y=None,
                     ord=np.inf, clip_min=None, clip_max=None, defense_models=None, **kwargs):
        """
        Take in a dictionary of parameters and applies attack-specific checks
        before saving them as attributes.
